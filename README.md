...
Проект разворачивается локально одной командой благодаря `Makefile`. 
Убедитесь, что у вас установлены Docker и Node.js.

```bash
git clone https://github.com/stacsmeh/soui-qa-automation-hub.git
...
1. **API-автотесты (Postman + Newman) с генерацией HTML-отчета:**
   ```bash
   make test-api
   ```
